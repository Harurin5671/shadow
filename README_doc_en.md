...
│   ├── AppRouter.swift          # App navigation (State-based)
│   ├── AppView.swift            # Root view switcher
│   └── shadowApp.swift          # @main entry point
├── Core/                        # Core business logic and infrastructure
│   ├── Application/             # Bounded contexts and repositories
│   │   ├── Repositories/
│   │   │   └── RoomRepository.swift
│   │   └── Services/
│   │       └── NotificationManager.swift
│   ├── Extensions/              # Domain-specific Swift extensions
│   │   └── NotificationCenter+RoomEvents.swift
│   └── Infrastructure/          # Technical infrastructure layer
│       ├── Crypto/              # E2EE engine and orchestrator
│       │   ├── CryptoManager.swift
│       │   └── CryptoService.swift
│       ├── DependencyInjection/
│       │   └── DIContainer.swift
│       ├── Network/             # Real-time communication (Socket.io)
│       │   ├── SocketEvent.swift
│       │   ├── SocketService.swift
...
├── DesignSystem/                # Visual foundation and components
...
│   ├── TextStyles.swift
│   └── Typography.swift
├── Features/                    # Domain features (MVVM)
│   ├── Chat/                    # Secure messaging interface
│   ├── Connection/              # Connectivity states
│   ├── Destruction/             # Auto-deletion logic
│   ├── Home/                    # Dashboard and room management
│   ├── RoomCreation/           # Flow to host new rooms
│   ├── RoomJoin/               # Flow to join existing rooms
│   ├── Onboarding/             # First-run experience
│   └── Settings/               # User preferences
├── Resources/                   # Assets and Localizable strings
│   ├── Localizable.xcstrings    # Multi-language support
│   └── Assets.xcassets
...
### 3. Dependency Injection (`DIContainer.shared`)

Using a Thread-Safe Singleton for Dependency Management. All services are initialized lazily or during startup to be provided via `@Observable`.

- `SocketService`: Real-time WebSocket management.
- `RoomRepository`: Application-level room cache and sync.
- `StorageService`: Unified interface for Keychain (Sensitive) and LocalStorage (Prefs).
- `CryptoService`: Low-level cryptographic primitives (ECDH, AES-256-GCM).
- `CryptoManager`: High-level E2EE orchestrator.
- `NotificationManager`: Centralized event-to-notification mapper.
...
### CryptoManager - E2EE Orchestration

The `CryptoManager` acts as an orchestrator that simplifies complex cryptographic flows into a clear API for the ViewModels.

#### 1. **Automated Key Exchange**

When a participant joins a room, `CryptoManager` handles the public key broadcast and shared secret generation automatically.

#### 2. **Room Key Distribution (Hosted Flow)**

Shadow uses a "Hosted Keys" model where the room creator generates the master `RoomKey` (AES-256) and shares it individually with each participant via E2EE (ECDH-wrapped).

```swift
// Creator flow in CryptoManager
...
Encryption is handled transparently before sending, and decryption occurs upon receipt.

```swift
// Sending a message (ChatViewModel)
